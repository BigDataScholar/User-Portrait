...
object MarriageTag {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 3. 读取MySQL数据库的四级标签
    val fourTagsDS: Dataset[Row] = mysqlConn.select("id","rule").where("id=115")
    ...
    //4. 读取mysql数据库的五级标签
    // 匹配职业
    val fiveTagsDS: Dataset[Row] = mysqlConn.select("id","rule").where("pid=115")
    ...
    val jobNewTags : DataFrame = hbaseDatas.select('id.as ("userId"),getUserTags('marriage).as("tagsId"))
    jobNewTags.show(5)
